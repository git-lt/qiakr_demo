 ...
 */
define(['common','cookie','validate'],function(){
	/**
	 * Utils
	 */
	var Utils={
	...
	avalon.filters.placeholderImg= function(url, type, width, height){
	    ...
	    var width = width || 80, placeholderImg = '';
	    switch(type){
...
$.fn.setTheadFixed = function(options){
    ...
    s.find("thead:first").find("th:visible").each(function(i,e){
        h += '<th style="width:'+$(e).width()+'px" class="'+($(e).attr("class")||"")+'">'+$(e).html()+'</th>';
    });
    h += '</tr></thead></table>';
